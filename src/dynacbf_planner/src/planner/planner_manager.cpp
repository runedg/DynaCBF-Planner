// 实现规划管理器的参数初始化、局部目标选择、轨迹生成和安全性检查。
// 将搜索路径转换为优化 B 样条，并维护连续可执行的局部轨迹。
// #include <fstream>
...
namespace dynacbf
{
  ...
  void SCANPlannerManager::initPlanModules(rclcpp::Node *node, PlanningVisualization::Ptr vis)
  {
    ...
    // 移植修正：上游用 -1.0 哨兵默认值读取上述参数，缺键或写入非法值时既不报错也不校验，
    // 会让规划管理器在错误的速度/加速度/加加速度上限下生成"看似可行"的轨迹（可行性检查
    // 因此失效，或控制点间距非法导致 B 样条退化）。这里补齐有限性与范围校验：越界（含缺键
    // 留下的 -1.0 哨兵）直接抛异常并打印具体键名，由 dynacbf_planner_node 捕获后以 FATAL
    // 退出（非法安全参数必须导致启动失败，不能静默采用危险值）。
    const std::pair<const char *, double> positive_manager_params[] = {
        ...
        {"manager.planning_horizon", pp_.planning_horizon_},
    };
    for (const auto &param : positive_manager_params)
    {
      if (!std::isfinite(param.second) || param.second <= 0.0)
        throw std::runtime_error(std::string("[SCANPlannerManager] invalid parameter '") + param.first +
                                 "': must be finite and > 0, got " + std::to_string(param.second));
    }

    const std::pair<const char *, double> non_negative_manager_params[] = {
        {"manager.feasibility_tolerance", pp_.feasibility_tolerance_},
        {"optimization.vel_tolerance", pp_.vel_tolerance_},
        {"optimization.acc_tolerance", pp_.acc_tolerance_},
    };
    for (const auto &param : non_negative_manager_params)
    {
      if (!std::isfinite(param.second) || param.second < 0.0)
        throw std::runtime_error(std::string("[SCANPlannerManager] invalid parameter '") + param.first +
                                 "': must be finite and >= 0, got " + std::to_string(param.second));
                                        ...
                                        Eigen::Vector3d local_target_vel, bool flag_polyInit, bool flag_randomPolyTraj)
  {
    ...
    /*** STEP 1: INIT ***/
    double ts = (start_pt - local_target_pt).norm() > 0.1 ? pp_.ctrl_pt_dist / pp_.max_vel_ * 2.0 : pp_.ctrl_pt_dist / pp_.max_vel_ * 5; // pp_.ctrl_pt_dist / pp_.max_vel_ is too tense, and will surely exceed the acc/vel limits
  ...
  bool SCANPlannerManager::checkDynamicFeasibility(UniformBspline position_traj)
  {
    ...
    // 工程裕量：优化器的可行性代价建立在**控制点差分**上，而本检查评估的是
    // **连续样条导数**，实测连续峰值比控制点估计高 4%~21%。不给裕量时会出现
    // "永远差一点点"的死循环（实测 1000 次重规划全部失败、机器人无法启动）。
    // 注意：这只影响名义轨迹的激进程度，安全性由控制器的 D-CBF 独立保证。
    const double kContinuityMargin = 2.0;
    const double vel_limit = (pp_.max_vel_ + pp_.vel_tolerance_) * kContinuityMargin;
    const double acc_limit = (pp_.max_acc_ + pp_.acc_tolerance_) * kContinuityMargin;
