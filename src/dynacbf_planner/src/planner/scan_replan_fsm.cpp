// 实现 SCAN-Planner 重规划状态机及不同导航模式下的目标管理。
// 根据里程计、当前轨迹和碰撞风险触发新规划、重规划或紧急停止。
...
namespace dynacbf
{
  ...
  void SCANReplanFSM::init(rclcpp::Node *node)
  {
    ...
    // 移植新增：外部重规划请求状态复位（时间戳使用节点时钟，避免 rclcpp::Time 时钟类型不一致）
    replan_request_active_ = false;
    has_replan_request_time_ = false;
    replan_request_stamp_ = node_->now();
    last_replan_request_time_ = node_->now();
    ...
    // 移植新增：外部重规划请求的限频与防抖参数
    replan_request_min_interval_ = load_parameter<double>(node_, "fsm.replan_request_min_interval", 2.0);
    replan_request_hold_ = load_parameter<double>(node_, "fsm.replan_request_hold", 0.5);
    ...
    // 移植修正：上游用 -1.0 哨兵默认值读取上述 fsm 参数，缺键或写入非法值时既不报错也不校验，
    // 会让状态机在错误的阈值/视界下运行（例如 thresh_replan 为负时"抑制重规划"判定完全失效，
    // 或 planning_horizon 非法导致目标被截断到错误位置）。这里补齐有限性与范围校验：越界
    // （含缺键留下的 -1.0 哨兵）直接抛异常并打印具体键名，由 dynacbf_planner_node 捕获后以
    // FATAL 退出（非法安全参数必须导致启动失败，不能静默采用危险值）。
    const std::pair<const char *, double> non_negative_fsm_params[] = {
        {"fsm.thresh_replan", replan_thresh_},
        {"fsm.thresh_no_replan", no_replan_thresh_},
        {"fsm.replan_request_min_interval", replan_request_min_interval_},
    };
    for (const auto &param : non_negative_fsm_params)
    {
      if (!std::isfinite(param.second) || param.second < 0.0)
        throw std::runtime_error(std::string("[FSM] invalid parameter '") + param.first +
                                 "': must be finite and >= 0, got " + std::to_string(param.second));
    }

    const std::pair<const char *, double> positive_fsm_params[] = {
        {"fsm.planning_horizon", planning_horizon_},
        {"fsm.emergency_time", emergency_time_},
        {"fsm.replan_request_hold", replan_request_hold_},
    };
    for (const auto &param : positive_fsm_params)
    {
      if (!std::isfinite(param.second) || param.second <= 0.0)
        throw std::runtime_error(std::string("[FSM] invalid parameter '") + param.first +
                                 "': must be finite and > 0, got " + std::to_string(param.second));
    ...
    if (no_replan_thresh_ > replan_thresh_)
      throw std::runtime_error("[FSM] invalid parameters: 'fsm.thresh_no_replan' (" +
                               std::to_string(no_replan_thresh_) +
                               ") must be <= 'fsm.thresh_replan' (" +
                               std::to_string(replan_thresh_) + ")");
    ...
    // 移植新增：外部重规划请求订阅（限频 + 保持窗口防抖，见 replanRequestCallback）
    ...
    bspline_pub_ = node_->create_publisher<dynacbf_msgs::msg::Bspline>("planning/bspline", 10);
    data_disp_pub_ = node_->create_publisher<dynacbf_msgs::msg::DataDisp>("planning/data_display", 100);
  ...
  // 移植新增：外部重规划请求回调。
  // 触发条件：话题 planning/replan_request 收到 std_msgs/Bool 且 data == true，
  //   并且（若此前接受过请求）距上次被接受的请求已超过 fsm.replan_request_min_interval 秒。
  // 防抖：请求被接受后 replan_request_hold 秒内保持请求态，由 FSM 主循环调用
  //   consumeExternalReplanRequest() 消费一次；窗口结束后自动失效，重复的 true 不会反复触发。
  // data == false 不改变任何状态（该话题只用于“请求一次重规划”）。
  void SCANReplanFSM::replanRequestCallback(const std_msgs::msg::Bool::ConstSharedPtr &msg)
  {
    ...
    if (has_replan_request_time_ &&
        (now - last_replan_request_time_).seconds() < replan_request_min_interval_)
    {
      RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
                           "[FSM] replan request throttled: min interval is %.2f s",
                           replan_request_min_interval_);
      return;
    }

    has_replan_request_time_ = true;
    last_replan_request_time_ = now;
    replan_request_stamp_ = now;
    replan_request_active_ = true;
    RCLCPP_INFO(node_->get_logger(), "[FSM] external replan request accepted (hold %.2f s)",
                replan_request_hold_);
  }

  // 移植新增：外部重规划请求是否仍处于保持窗口内；窗口结束后自动清除请求态。
  bool SCANReplanFSM::externalReplanRequestActive()
  {
    if (!replan_request_active_)
      return false;

    if ((node_->now() - replan_request_stamp_).seconds() > replan_request_hold_)
    {
      replan_request_active_ = false;
      return false;
    }
  ...
  // 移植新增：消费一次保持中的外部重规划请求。
  bool SCANReplanFSM::consumeExternalReplanRequest()
  {
    if (!externalReplanRequestActive())
      return false;
  ...
  {
    // 移植新增：消费外部重规划请求（话题 planning/replan_request）。
    // 走与 navi_mode==1 收到新目标点相同的重规划路径：正在执行轨迹时切到 REPLAN_TRAJ，
    // 尚无轨迹但已锁定目标时切到 GEN_NEW_TRAJ；其余状态（如 INIT / EMERGENCY_STOP）不打断。
    if (consumeExternalReplanRequest())
    {
      if (!have_target_)
      {
        RCLCPP_WARN(node_->get_logger(), "[FSM] replan request ignored: no active target");
      }
      else if (exec_state_ == EXEC_TRAJ)
      {
        changeFSMExecState(REPLAN_TRAJ, "REQ");
      }
      else if (exec_state_ == WAIT_TARGET)
      {
        trigger_ = true;
        init_pt_ = odom_pos_;
        changeFSMExecState(GEN_NEW_TRAJ, "REQ");
      }
      else
      {
        RCLCPP_INFO(node_->get_logger(), "[FSM] replan request ignored: FSM is not executing a trajectory");
      }
    }
  ...
  bool SCANReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    if (plan_success)
    {
      ...
      /* publish traj */
      dynacbf_msgs::msg::Bspline bspline;
      bspline.order = 3;
