// 实现地图参数加载、传感器融合、占据更新、膨胀维护和点云可视化。
// 将世界坐标观测融合到滑动体素缓冲，并向搜索与优化模块提供安全占用层。
...
// 移植修正：安全相关数值参数的有限性与范围校验。
// 上游的 load_parameter 只提供哨兵默认值（-1.0 / -1），缺键或写入非法值时既不报错也不校验，
// 会静默算出错误的栅格几何（分辨率/地图尺寸）、失效的膨胀层与占据阈值（把障碍判成空闲）、
// 或造成除零。这里改为：非有限或越界即抛异常并打印具体键名，由 dynacbf_planner_node 捕获后
// 以 RCLCPP_FATAL + 退出码 1 结束 —— 非法安全参数必须导致启动失败，不能静默采用危险值。
double require_finite_in_range(const std::string &name, double value, double min_value, double max_value)
{
  if (!std::isfinite(value))
    throw std::runtime_error("[GridMap] invalid parameter '" + name + "': not a finite number");

  if (value < min_value || value > max_value)
    throw std::runtime_error("[GridMap] invalid parameter '" + name + "': " + std::to_string(value) +
...
void GridMap::initMap(rclcpp::Node *node)
{
  ...
  // 移植修正：在推导任何派生量（resolution_inv_ / log-odds / map_size_ / 膨胀偏移）之前，
  // 先校验所有安全相关参数，避免上游"缺键不报错、只算错几何"的行为。校验口径见文件顶部
  // require_finite_in_range 的说明；越界或非有限值会让节点启动失败并打印具体键名。
  require_finite_in_range("grid_map.resolution", mp_.resolution_, 1e-3, 1.0);
  require_finite_in_range("grid_map.sliding_map_size_x", x_size, 1e-3, 1e4);
  require_finite_in_range("grid_map.sliding_map_size_y", y_size, 1e-3, 1e4);
  require_finite_in_range("grid_map.sliding_map_size_z", z_size, 1e-3, 1e4);
  require_finite_in_range("grid_map.local_update_range_x", mp_.local_update_range_(0), 1e-3, x_size);
  require_finite_in_range("grid_map.local_update_range_y", mp_.local_update_range_(1), 1e-3, y_size);
  require_finite_in_range("grid_map.local_update_range_z", mp_.local_update_range_(2), 1e-3, z_size);
  require_finite_in_range("grid_map.obstacles_inflation_z_up", mp_.obstacles_inflation_z_up, 0.0, z_size);
  require_finite_in_range("grid_map.obstacles_inflation_z_down", mp_.obstacles_inflation_z_down, 0.0, z_size);
  require_finite_in_range("grid_map.double_cylinder_radius", mp_.double_cylinder_radius_, 0.0, 1e3);
  require_finite_in_range("grid_map.double_cylinder_offset", mp_.double_cylinder_offset_, 0.0, 1e3);
  require_finite_in_range("grid_map.map_sliding_thresh", mp_.map_sliding_thresh_, 0.0, 1e3);
  require_finite_in_range("grid_map.ground_height", mp_.ground_height_, -1e4, 1e4);
  require_finite_in_range("grid_map.max_ray_length", mp_.max_ray_length_, 1e-3, 1e4);

  // 概率阈值：必须严格落在 (0, 1) 开区间内（logit() 在 0/1 处发散），且交叉关系必须成立，
  // 否则 free/occupied 判定会整体反转 —— 规划器会把障碍物当成可通行区域。
  require_finite_in_range("grid_map.p_hit", mp_.p_hit_, 1e-6, 1.0 - 1e-6);
  require_finite_in_range("grid_map.p_miss", mp_.p_miss_, 1e-6, 1.0 - 1e-6);
  require_finite_in_range("grid_map.p_min", mp_.p_min_, 1e-6, 1.0 - 1e-6);
  require_finite_in_range("grid_map.p_max", mp_.p_max_, 1e-6, 1.0 - 1e-6);
  require_finite_in_range("grid_map.p_occ", mp_.p_occ_, 1e-6, 1.0 - 1e-6);

  if (!(mp_.p_min_ < mp_.p_occ_ && mp_.p_occ_ < mp_.p_max_))
    throw std::runtime_error(
        "[GridMap] invalid parameters: require 'grid_map.p_min' < 'grid_map.p_occ' < 'grid_map.p_max', got " +
        std::to_string(mp_.p_min_) + " / " + std::to_string(mp_.p_occ_) + " / " + std::to_string(mp_.p_max_));

  if (!(mp_.p_hit_ > mp_.p_miss_))
    throw std::runtime_error(
        "[GridMap] invalid parameters: require 'grid_map.p_hit' > 'grid_map.p_miss', got " +
        std::to_string(mp_.p_hit_) + " / " + std::to_string(mp_.p_miss_));

  // 深度相机参数只在 sensor_type == "depth" 时参与投影，因此仅在深度模式下校验，
  // 避免纯激光雷达配置被无关键阻断。
  if (mp_.sensor_type_ == "depth")
  {
    require_finite_in_range("grid_map.fx", mp_.fx_, 1e-6, 1e6);
    require_finite_in_range("grid_map.fy", mp_.fy_, 1e-6, 1e6);
    require_finite_in_range("grid_map.cx", mp_.cx_, -1e6, 1e6);
    require_finite_in_range("grid_map.cy", mp_.cy_, -1e6, 1e6);
    require_finite_in_range("grid_map.depth_filter_mindist", mp_.depth_filter_mindist_, 1e-6, 1e4);
    require_finite_in_range("grid_map.depth_filter_maxdist", mp_.depth_filter_maxdist_, 1e-6, 1e4);
    require_finite_in_range("grid_map.k_depth_scaling_factor", mp_.k_depth_scaling_factor_, 1e-6, 1e9);
    require_finite_in_range("grid_map.skip_pixel", static_cast<double>(mp_.skip_pixel_), 1.0, 1024.0);
    require_finite_in_range("grid_map.depth_filter_margin", static_cast<double>(mp_.depth_filter_margin_),
                            0.0, 1e4);

    if (mp_.depth_filter_mindist_ >= mp_.depth_filter_maxdist_)
      throw std::runtime_error(
          "[GridMap] invalid parameters: require 'grid_map.depth_filter_mindist' < "
          "'grid_map.depth_filter_maxdist', got " +
          std::to_string(mp_.depth_filter_mindist_) + " / " + std::to_string(mp_.depth_filter_maxdist_));
  }
