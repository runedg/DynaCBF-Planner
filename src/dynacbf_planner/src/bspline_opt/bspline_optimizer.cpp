// 实现 B 样条控制点初始化、碰撞回弹和多目标轨迹优化流程。
// 分段生成避障方向后，使用 L-BFGS 输出世界坐标中的优化控制点。
#include "dynacbf_planner/bspline_opt/bspline_optimizer.h"
// 移植修正：上游此处 include 了死类 GradientDescentOptimizer（全仓库无任何实例化点），
// 本项目不迁移该文件，因此移除该 include。
#include <algorithm>
#include <chrono>
#include <cmath>
#include <stdexcept>
...
namespace dynacbf
{
  ...
  void BsplineOptimizer::setParam(rclcpp::Node *node)
  {
    ...
    // 移植修正：上游用 -1.0 哨兵默认值读取这些参数，缺键或写入非法值时既不报错也不校验，
    // 会让优化器在错误的权重/速度上限下“正常”求解（把不可行轨迹当成可行、或碰撞代价失效）。
    // 这里补齐有限性与范围校验：越界（含缺失键留下的 -1.0 哨兵）直接抛异常并打印具体键名，
    // 由 dynacbf_planner_node 捕获后以 FATAL 退出（非法安全参数必须导致启动失败）。
    const std::pair<const char *, double> non_negative_weights[] = {
        {"optimization.lambda_smooth", lambda1_},
        {"optimization.lambda_collision", lambda2_},
        {"optimization.lambda_feasibility", lambda3_},
        {"optimization.lambda_fitness", lambda4_},
        {"optimization.dist0", dist0_},
    };
    for (const auto &weight : non_negative_weights)
    {
      if (!std::isfinite(weight.second) || weight.second < 0.0)
        throw std::runtime_error(std::string("[BsplineOptimizer] invalid parameter '") + weight.first +
                                 "': must be finite and >= 0, got " + std::to_string(weight.second));
    }

    const std::pair<const char *, double> positive_limits[] = {
        {"optimization.max_vel", max_vel_},
        {"optimization.max_acc", max_acc_},
    };
    for (const auto &limit : positive_limits)
    {
      if (!std::isfinite(limit.second) || limit.second <= 0.0)
        throw std::runtime_error(std::string("[BsplineOptimizer] invalid parameter '") + limit.first +
                                 "': must be finite and > 0, got " + std::to_string(limit.second));
    }

    if (order_ < 1 || order_ > 10)
      throw std::runtime_error("[BsplineOptimizer] invalid parameter 'optimization.order': must be in [1, 10], got " +
  ...
  std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
      ...
      for (double a = 1.0; a >= 0.0; a -= step_size)
      {
        ...
        if (flag_got_start && flag_got_end)
        {
          ...
          if (in_id >= 0 && out_id >= in_id && out_id < init_points.cols())
            ...
            RCLCPP_WARN(rclcpp::get_logger("dynacbf_planner.bspline_opt"),
    ...
    for (size_t i = 0; i < segment_ids.size(); ++i)
    {
      ...
      if (ret == ASTAR_RET::SUCCESS)
      {
        ...
        if (path.size() < 2)
        {
          RCLCPP_WARN(rclcpp::get_logger("dynacbf_planner.bspline_opt"), "A-star path has fewer than 2 points");
          return a_star_paths;
        }
      ...
      {
        RCLCPP_ERROR(rclcpp::get_logger("dynacbf_planner.bspline_opt"), "A-star failed; aborting optimization");
        return a_star_paths;
      }
    }
  ...
  bool BsplineOptimizer::check_collision_and_rebound(void)
  {
    ...
    for (int i = order_ - 1; i <= i_end; ++i)
    {
      ...
      if (occ)
      {
        ...
        {
          RCLCPP_ERROR(rclcpp::get_logger("dynacbf_planner.bspline_opt"), "The robot is inside an obstacle");
          in_id = 0;
        }
    ...
    if (flag_new_obs_valid)
    {
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        {
          RCLCPP_ERROR(rclcpp::get_logger("dynacbf_planner.bspline_opt"), "A-star error");
          segment_ids.erase(segment_ids.begin() + i);
          i--;
        }
      }
      ...
      for (size_t i = 0; i < segment_ids.size(); ++i)
      {
        ...
        if (got_intersection_id >= 0)
        {
          ...
          RCLCPP_WARN(rclcpp::get_logger("dynacbf_planner.bspline_opt"), "Failed to generate rebound direction");
      }
  ...
  bool BsplineOptimizer::refine_optimize()
  {
    ...
    do
    {
      ...
      {
        RCLCPP_ERROR(rclcpp::get_logger("dynacbf_planner.bspline_opt"),
