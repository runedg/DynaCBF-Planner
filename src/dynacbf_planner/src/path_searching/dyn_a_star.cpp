// 实现动态 A* 的坐标映射、邻域扩展、碰撞检查和路径回溯。
// 在栅格索引空间搜索，再将结果转换回世界坐标路径。
#include <dynacbf_planner/path_searching/dyn_a_star.h>
#include <algorithm>
#include <chrono>
#include <cmath>
...
bool AStar::ConvertToIndexAndAdjustStartEndPoints(Vector3d start_pt, Vector3d end_pt, Vector3i &start_idx, Vector3i &end_idx)
{
    ...
    if (occ)
    {
        ...
        do
        {
            ...
            if (occ == -1)
            {
                RCLCPP_WARN(rclcpp::get_logger("dynacbf_planner.path_searching"), "[Astar] Start point outside the map region.");
                return false;
            }
        } while (occ);
    }
    ...
    if (occ)
    {
        ...
        do
        {
            ...
            if (occ == -1)
            {
                RCLCPP_WARN(rclcpp::get_logger("dynacbf_planner.path_searching"), "[Astar] End point outside the map region.");
                return false;
            }
        } while (occ);
    }
...
ASTAR_RET AStar::AstarSearch(const double step_size, Vector3d start_pt, Vector3d end_pt)
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("dynacbf_planner.path_searching"),
    ...
    while (!openSet_.empty())
    {
        ...
        {
            RCLCPP_WARN(rclcpp::get_logger("dynacbf_planner.path_searching"),
