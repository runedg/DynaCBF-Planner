...
struct Ellipse
{
	...
	// 【本项目修正】原版此处没有默认值。而 get_ellipse_array() 里是
	// `Ellipse ellipse;` 之后从不设置 label（label 由后续 KM.tracking() 赋），
	// 一旦某个椭圆没被赋上，未初始化的垃圾值就会传到 ab_variance_calculation()，
	// 那里用 list[label - 1] 当下标（vector::operator[] 无边界检查）→ 越界崩溃，
	// 表现为 SIGFPE / SIGSEGV（实测 local_map exit code -8）。
	// 0 表示"尚未分配 label"，调用方需跳过。
	int label = 0;	  // label of obstacle
...
vector<Ellipse> get_ellipse_array(std::vector<Obstacle> &seg_obs, const grid_map::GridMap &localmap_obs, const LocalMapParam &MapParam)
{
	...
	for (const auto &obs : seg_obs)
	{
		if (obs.size() > 500)
			continue;
		int x_min = localmap_x_size * _inv_resolution, y_min = localmap_y_size * _inv_resolution, x_max = 0, y_max = 0;
		...
		{
			if (pt(0) < 0 || pt(1) < 0 ||
				pt(0) >= localmap_x_size / resolution ||
				pt(1) >= localmap_y_size / resolution)
				continue;
		...
		if (!get_min_area_ellipse(pts, ellipse))
		{
			...
			ellipse.theta = std::atan2(
				static_cast<double>(y_max - y_min),
				static_cast<double>(x_max - x_min)) + M_PI / 2;
		}
...
void ab_variance_calculation(vector<Ellipse> &input_vector)
{
	...
	for (auto &input : input_vector)
	{
		...
		// 【本项目修正】label <= 0 表示尚未分配（见 Ellipse 结构体说明）。
		// 此时 list[label - 1] 会越界访问（vector::operator[] 不做边界检查）→ 直接崩溃。
		// 原版能跑是因为视野内目标足够多、KM.tracking 总会赋上正值；
		// 目标极少时（例如空场景）未赋值的椭圆才会走到这里。
		if (label <= 0)
			continue;
