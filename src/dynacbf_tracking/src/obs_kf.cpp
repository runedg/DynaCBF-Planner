...
private:
    void obscb(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
    {
        ...
        if (msg->data.empty())
        {
            obs_pub_->publish(std_msgs::msg::Float32MultiArray());
            visEllipse({});
            return;
        }
        ...
        {
            RCLCPP_WARN(this->get_logger(), "reject malformed obstacle array with %zu values", msg->data.size());
            return;
        }
        ...
        for (int i = 0; i < num; i++)
        {
            ...
            {
                RCLCPP_WARN(this->get_logger(), "ignore out-of-range obstacle label %d", flag + 1);
                continue;
            }
